...
class PingPongContract {
  ...
  constructor() {
    ...
    this.readOnlyProvider = new ethers.JsonRpcProvider(ACTIVE_PROVIDER_URL);
  ...
  async createMatch(opponent: string, stakeAmount: string): Promise<any> {
    ...
    try {
      console.log(`Creating match with opponent: ${opponent}, stake: ${stakeAmount} ETH`);
  ...
  async joinMatch(gameId: string, stakeAmount: string): Promise<any> {
    ...
    try {
      ...
      if (stakeAmount !== requiredStake) {
        throw new Error(`Stake amount must be ${requiredStake} ETH`);
      }
